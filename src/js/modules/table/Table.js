...
export default class Table extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.columns.length === 0 && nextProps.columns.length > 0 && this.state.initialSortProp !== '') {
      this.setState({lastSortProp: this.state.initialSortProp, lastSortAsc: 'asc', columns: nextProps.columns}, () => {
  ...
  componentDidMount() {
    this.setState({
      ...
      if (this.state.columns.length > 0 && this.state.initialSortProp !== '') {
        this.sortTable(this.state.initialSortProp);
        this.setState({lastSortProp: this.state.initialSortProp, lastSortAsc: 'asc'});
      }
  ...
  sortTable(propName, pr) {
    let reverse = propName === this.state.lastSortProp && this.state.lastSortAsc === 'asc';
    let items = this.state.items;

    if (propName !== this.state.lastSortProp) {
      items = items.sort(this.sortArrayBy(propName, reverse, pr));
      this.setState({items, lastSortAsc: 'asc', lastSortProp: propName}, () => {
      });
    } else {
      if (this.state.lastSortAsc === 'desc') {
        ...
        items = items.sort(this.sortArrayBy(propName, reverse, pr));
        this.setState({items, lastSortAsc: 'desc', lastSortProp: propName}, () => {
        });
      }
    }
  }
  ...
  getIconForSort(propName) {
    if (propName === this.state.lastSortProp) {
      return this.state.lastSortAsc === 'asc' ? (<i className="mdi mdi-long-arrow-down"></i>) : (
        <i className="mdi mdi-long-arrow-up"></i>)
    } else {
      return null;
    }
  }
  ...
  searchFor(toSearch, items, props) {
    ...
    for (let i = 0; i < items.length; i++) {
      for (let j = 0; j < props.length; j++) {
        if (this.trimString(items[i][props[j]]).toLowerCase().indexOf(toSearch.toLowerCase()) !== -1) {
  ...
  getItems() {
    ...
    if (searchProps.length > 0 && this.state.searchString.length > 0) {
      return this.searchFor(this.state.searchString, this.state.items, this.props.searchProps);
    } else {
  ...
  render() {
    ...
    return (
      <div
        className={'table ' + (this.props.tableClasses && (this.props.tableClasses(this.state.data, this.state.index)))}
...

        {
          ...
          <li className="dropdown toggle-columns-container"><a href="#" data-toggle="dropdown" role="button" aria-expanded="false" className="dropdown-toggle">Esconder/Exibir colunas <span className="mdi mdi-caret-down"></span></a>
            <ul role="menu" className="dropdown-menu" style={{marginBottom: '20px'}}>
              {this.state.columns.map((column, index) => {
                return (
                  <li style={{marginLeft: '15px'}} key={index}>
                    <input type="checkbox" id={(this.props.id || 'roove') + '-check-column-visible' + index}
                           name={(this.props.id || 'roove') + '-check-column-visible' + index}
                           onChange={this.changeColumnsVisible.bind(this,column)}
                           checked={_.includes(this.state.columnsVisible,column)}/>
                    <label htmlFor={(this.props.id || 'roove') + '-check-column-visible' + index} className="lg-ml-10">{column.name}</label>
                  </li>
                );
              })}
            </ul>
